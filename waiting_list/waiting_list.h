...
public:
  ...
  virtual int size() = 0;
  virtual void report() {};
  virtual std::optional<YPoint> try_pop() {
    if (size() == 0)
      return std::nullopt;
    return pop();
  }
...
class SyncedStack : public Stack {
public:
  SyncedStack() : Stack() {};
  YPoint pop() override {
...
class SyncedQueue : public Queue {
public:
  SyncedQueue() : Queue() {};
  YPoint pop() override {
    std::lock_guard<std::mutex> lock(mtx);
...
class PriorityQueue : public WaitingListI {
...
public:
  ...
  virtual void unpop(int note = 0) override {
    insert(popped, true, note + popped_count);
  }
...
template <bool lower = false, bool ascend = false>
class List : public WaitingListI {
  using element_t = YPoint_priority<lower, ascend>;
