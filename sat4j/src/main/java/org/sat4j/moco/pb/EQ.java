...
/**
 * Representation of an EQual (EQ) constraint.
 * @author Miguel Terra-Neves
 */
public class EQ extends PBConstr {
     ...
     */
    EQ(IVecInt lits, IVec<Real> coeffs, Real rhs) { super(lits, coeffs, rhs); }

    @Override
    protected Real getActivatorCoeff() {
        throw new RuntimeException("equality constraints do not support activator literals");
    }
    
    @Override
    public boolean violatedBy(Real val) { return !val.equals(getRHS()); }

    @Override
    IConstr addToSolver(IPBSolver solver) throws ContradictionException {
        if (getLHS().allUnitCoeffs()) {
            try {
                return solver.addExactly(getLits(), getRHS().asIntExact());
            }
            catch (ArithmeticException ae) {
                throw new ContradictionException(
                        "exactly constraint with unit coefficients and non integer RHS is unsatisfiable", ae);
            }
        }
    ...
    protected IConstr addScaledToSolver(IPBSolver solver, IVecInt lits, IVec<BigInteger> coeffs, BigInteger rhs)
        ...
        return solver.addExactly(lits, coeffs, rhs);
    }
