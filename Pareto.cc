...
thread_local openwbo::YPoint pareto::max{};
thread_local openwbo::YPoint pareto::min{};
thread_local uint64_t pareto::hv_total = 0;
  ...
  try {
    for (auto &el : sol)
      if (dominates(el.second.first, osol)) {
        return true;
        std::cout << "c new " << osol << " dominated by " << el.second.first
                  << std::endl;
      }
