...
void Solution::pop() { mods.erase(--mods.end()); }
...
void MOCO::transferToSolution() {
  solution().push(Model{(*(--effsols.end())).first,
                        (Model::size_type)getFormula()->nVars()});
}
      ...
      if (maxsat_formula->getFormat() == _FORMAT_PB_) {
        for (int i = 0; i < maxsat_formula->nVars(); i++) {
          indexMap::const_iterator iter =
              maxsat_formula->getIndexToName().find(i);
...
void MOCO::printMyStats() {
  ...
  //         maxsat_formula->nHard()); runstats[_nencvars_] = S->nVars() -
  //         maxsat_formula->nVars(); runstats[_nencclauses_] = S->nClauses() -
  ...
  printf("clrunstats %18s %12s %12s %12s %12s %12s %18s %12s %18s %18s %8s %8s "
         "%8s\n",
         ...
         "nobj");
  printf("crunstats %12d %12d %12d %12d %12d %12d %18d %12d %18d %18d %18d "
         "%18.4f %10d\n",
         runstats[_nsatcalls1stSol_], runstats[_nsatcalls_], runstats[_ncalls_],
         runstats[_neffsols_], runstats[_nnondom_], runstats[_nprobvars_],
         runstats[_nprobclauses_], runstats[_nencvars_],
         runstats[_nencclauses_], runstats[_nencrootvars_],
         runstats[_nreencodes_], repsilon, getFormula()->nObjFunctions());
  //
  //
  timestats[_totaltime_] = totalTime - initialTime;
  printf("cltimestats %12s %12s\n", "time_1stSol", "totaltime");
  printf("ctimestats %12.2f %12.2f\n", timestats[_time1stSol_],
         timestats[_totaltime_]);

  if (print_my_output) {
    FILE *file = fopen(stats_file, "w");
    fprintf(file, "# %18s %12s %12s %12s %12s %12s %18s %12s %18s %18s",
            "nsatcalls_1stSol", "nsatcalls", "ncalls", "n_eff_sols", "n_nondom",
            "n_prob_vars", "n_prob_clauses", "n_enc_vars", "n_enc_clauses",
            "n_enc_rootvars");
    fprintf(file, " %12s %12s %12s\n", "rapprox", "time_1stSol", "totaltime");

    fprintf(file, "  %18d %12d %12d %12d %12d %12d %18d %12d %18d %18d",
            runstats[_nsatcalls1stSol_], runstats[_nsatcalls_],
            runstats[_ncalls_], runstats[_neffsols_], runstats[_nnondom_],
            runstats[_nprobvars_], runstats[_nprobclauses_],
            runstats[_nencvars_], runstats[_nencclauses_],
            runstats[_nencrootvars_]);
    fprintf(file, " %12.4f %12.2f %12.2f\n", repsilon, timestats[_time1stSol_],
            timestats[_totaltime_]);

    fclose(file);
  }
}
void MOCO::saveEfficientSol(const vec<lbool> &currentModel,
                            const uint64_t *point, bool filter) {
  ...
  // TODO FIX: Na versao de enumerar todas as eficientes, a relacao entre nondom
  // e effsols sera de 1-N
  bool isdom = false;
  if (filter) {
    // estou a assumir que nenhum ponto em nondom domina o point, mas o point
    // pode dominar pontos em nondom
  ...
  if (!isdom) {
    ...
    // Only store the value of the variables that belong to the original MaxSAT
    // formula.
...
// na pratica, isto calcula o multiplicative epsilon indicator, em que o LBset e
// o conjunto de referencia e o nondom e o conjunto a ser avaliado
  ...
  for (size_t j = 0; j < LBset.size(); j++) {
    ...
    for (size_t i = 0; i < nondom.size(); i++) {
      ...
      for (int di = 0; di < d; di++) {
        ...
        // 2)
        if (LBset[j][di] > 0 && float(nondom[i][di]) / LBset[j][di] > ptepsi &&
            LBset[j][di] * (float(nondom[i][di]) / LBset[j][di]) >=
                LBset[j][di] + 1) {
...
bool MOCO::updateLowerBoundSet(const uint64_t *point, bool filter,
                               uint64_t ireencode) {
  uint64_t *pt;
...
printf("\n");*/
  ...
  if (filter) {
    for (size_t i = 0; i < LBset.size(); i++) {
      if (LBset[i][d] == ireencode && wdominates(LBset[i], point, d)) {
        isdom = true;
      } else if ((LBset[i][d] + 2 <= ireencode) ||
                 (LBset[i][d] == ireencode && wdominates(point, LBset[i], d))) {
        delete[] LBset[i];
  ...
  if (!isdom) {
    pt = new uint64_t[d + 1]; // a ultima posicao guarda a iteracao de reencode
