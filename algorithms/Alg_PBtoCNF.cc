...
void PBtoCNF::saveModel(vec<lbool> &currentModel) {
...
void PBtoCNF::saveSmallestModel(vec<lbool> &currentModel) {
  // assert (n_initial_vars != 0);
  assert(currentModel.size() != 0);
...
lbool PBtoCNF::identifyDisjointCores() {
  lbool res = l_True;
...
uint64_t PBtoCNF::coreMinCost(int c) {
...
bool PBtoCNF::satisfiedSoft(int i) {
...
void PBtoCNF::addMCSClause(vec<int> &unsatClauses) {
  ...
  if (getFormula()->nObjFunctions() == 1) {
    ...
    solver->addClause(
        getFormula()->getHardClause(getFormula()->nHard() - 1).clause);
    // solver->addClause(lits);
  }
}

void PBtoCNF::initUndefClauses(vec<int> &undefClauses) {
...
void PBtoCNF::basicCoreBasedSearch(int maxMCS = 10, bool maxSAT = false) {
  // Init Structures
  init();
...
void PBtoCNF::addBackboneLiterals(int index) {
...
bool PBtoCNF::findNextCoreBasedMCS() {
  vec<int> undefClauses;
  ...
  while (initCore < _cores.size() && _coreUnsatClauses[initCore].size() > 0) {
    for (int i = 0; i < _coreUnsatClauses[initCore].size(); i++)
      costModel +=
          getFormula()->getSoftClause(_coreUnsatClauses[initCore][i]).weight;
    initCore++;
  }
  ...
  for (int c = initCore; c < _cores.size(); c++) {
    ...
    while (undefClauses.size()) {
      ...
      if (res == l_False) {
        ...
        costModel -=
            getFormula()->getSoftClause(_coreSatClauses[c].last()).weight;
        ...
        while (i < undefClauses.size()) {
          if (satisfiedSoft(undefClauses[i])) {
            ...
            assumptions.push(
                ~(getFormula()->getSoftClause(undefClauses[i]).assumption_var));
...
void PBtoCNF::tmpBlockDominatedRegion(uint64_t *objix, int nObj, Lit tmplit) {
  vec<Lit> d_cl;
  d_cl.push(tmplit);
...
void PBtoCNF::tmpBlockDominatedRegion(const YPoint &yp, Lit tmplit) {
  int nObj = yp.size();
  // computing the indexes given the objective value. This should be abstracted
  // away...
  uint64_t objix[nObj];
...
void PBtoCNF::blockDominatedRegion(uint64_t *objix, int nObj) {
...
void PBtoCNF::blockDominatedRegion(const YPoint &yp) {
  int nObj = yp.size();
  uint64_t objix[nObj];
...
void PBtoCNF::activateLit(Lit lit) {
  vec<Lit> d_cl;
  d_cl.push(lit);
  solver->addClause(d_cl);
}

void PBtoCNF::getObjRootLits(uint64_t *objv, uint64_t *objix,
  ...
  for (int i = 0; i < nObj; i++) {
    if (objix[i] > 0 && (*objRootLits[i])[objix[i]].first != objv[i]) {
      ...
      objRootLits[i]->insert(exact_objix[i],
                             std::pair<uint64_t, Lit>(objv[i], l));
    } else {
...
void PBtoCNF::getNewObjRootLitsAndBlock(uint64_t *objv, uint64_t *objix,
                                        int nObj) {
  Lit l;
...
bool PBtoCNF::apObjectivesAreExact() {
  assert(approxMode == encoding::_ap_coeffs_);
  printf("c [PBtoCNF] is the approx function exact?\n");
...
bool PBtoCNF::MCSSearchMO(bool permanentBlock) {
#ifdef __DEBUG__
...
void PBtoCNF::update_assumpt_n_mcs(Lit mcsesBlockLit, uint64_t *objix,
...
void PBtoCNF::basicSearch(int maxMCS = 50, bool maxSAT = false) {
  // Init Structures
  init();
  ...
  if (enc_is_kp_based() || encoder.getPBEncoding() == _PB_GTE_ ||
    ...
    for (int i = 0; i < getFormula()->nObjFunctions(); i++) {
      ...
      if (enc_is_kp_based() || encoder.getPBEncoding() == _PB_GTE_) {
        ...
        if (!pb.empty()) {
          if (enc_is_kp_based()) {
            kps[i].setApproxRatio(solver, 1, 0);
...
shared_ptr<MOCOFormula> PBtoCNF::workFormula() { return wf; }
bool PBtoCNF::loadWorkFormula() {
...
//  encoding)
void PBtoCNF::addAssumptMOFormulation(bool omitRHS) {
...
void PBtoCNF::getPoint(uint64_t *ix, int d, uint64_t *out) {
  //     uint64_t * pt = (out == NULL) ? new uint64_t[d] : out;
  //     uint64_t * pt ;
  for (int di = 0; di < d; di++) {
    // pt[di] = objRootLits[di][ix[di]-1].first;
    // printf("[PBtoCNF::getPoint] ix[%d]: %lu\n", di, ix[di]);
...
void PBtoCNF::fixLBpoint(uint64_t *lb, uint64_t *ptdom, int d) {
  for (int di = 0; di < d; di++) {
...
void PBtoCNF::printRootLit(int d) {
  size_t maxvs = 5;
  for (int di = 0; di < d; di++) {
    printf("c [PBtoCNF] printRootLit[%d] (%lu)\n", di,
           (*objRootLits[di]).size());
    //         printf("obj %d:", di);
    for (size_t i = 0; i < (*objRootLits[di]).size(); i++) {
      if (i < maxvs || i > (*objRootLits[di]).size() - maxvs)
        printf(" [%lu] z%d", (*objRootLits[di])[i].first,
               var((*objRootLits[di])[i].second) + 1);
      else if (i == maxvs)
        printf(" ...");
    }
    printf("\n");
...
void PBtoCNF::printRootLitModel(int d) {
  printf("c [PBtoCNF] printRootLitModel\n");
  for (int di = 0; di < d; di++) {
    size_t i = 0;
    printf("obj %d:", di);
...
void PBtoCNF::evalModel(vec<lbool> &currentModel, uint64_t *objv,
                        uint64_t *ap_objv) {
  Lit l;
  for (int di = 0; di < getFormula()->nObjFunctions(); di++) {
...
void PBtoCNF::evalToIndex(const YPoint &yp, uint64_t *objix) {
  openwbo::evalToIndex(yp, objix, objRootLits);
}

void PBtoCNF::evalToIndex(uint64_t *objv, uint64_t *objix) {
  ...
  for (int di = 0; di < getFormula()->nObjFunctions(); di++) {
    ...
    if (objRootLits[di] != NULL)
      while (i <= (*objRootLits[di]).size() &&
             objv[di] >= (*objRootLits[di])[i].first) {
        objix[di] = i;
        i++;
      }
  }
...
Solver *PBtoCNF::buildSolverMO() {
  ...
  for (int i = 0; i < getFormula()->nPB(); i++) {
    encoding::Encoder *enc = new encoding::Encoder(
        _INCREMENTAL_NONE_, encoding, _AMO_LADDER_, pb_encoding); // AG
  ...
  for (int i = 0; i < getFormula()->nCard(); i++) {
    ...
    encoding::Encoder *enc = new encoding::Encoder(
        _INCREMENTAL_NONE_, encoding, _AMO_LADDER_, pb_encoding); // encoding
...
uint64_t PBtoCNF::getTighterUB(int di) {
  return getFormula()->getTighterUB(di);
}

uint64_t PBtoCNF::getTighterLB(int di) {
  return getFormula()->getTighterLB(di);
}
bool PBtoCNF::firstSolution() {
  if (first.model().size())
    return true;
...
bool PBtoCNF::updateMOFormulationIfSAT() {
...
bool PBtoCNF::updateMOFormulation() {
...
void PBtoCNF::consolidateSolution() {
  if (first.model().size())
...
void PBtoCNF::assumeDominatingRegion(uint64_t *objix, int nObj) {
  for (int di = 0; di < nObj; di++) {
    int j = objix[di];
    if (j > 0) {
...
int PBtoCNF::assumeDominatingRegion(const YPoint &yp) {
  auto nObj = yp.size();
  YPoint yp1 = yp;
  uint64_t objix[nObj];
  int pushed = 0;
...
PBObjFunction openwbo::filter(const PBObjFunction *pb, set<Lit> &vars) {
...
int PBtoCNF::blockStep(const YPoint &yp) {
  blockDominatedRegion(yp);
  return -1;
}

void openwbo::evalToIndex(const YPoint &yp, uint64_t *objix,
  ...
  for (uint64_t di = 0; di < yp.size(); di++) {
    ...
    size_t i = 1;
    while (i < (*objRootLits[di]).size() &&
           yp[di] + 1 >= (*objRootLits[di])[i].first) {
      objix[di] = i;
      i++;
    }
  }
}
int PBtoCNF::toggleConflictBudget(int limit) {
  if (limit > 0) {
    old_conflict_limit = conflict_limit;
    conflict_limit = limit;
    nConflicts = conflict_limit;
    return old_conflict_limit;
  } else if (limit < 0) {
    old_conflict_limit = conflict_limit;
    conflict_limit = -1;
    nConflicts = conflict_limit;
    ...
    conflict_limit = old_conflict_limit;
    nConflicts = conflict_limit;
