...
void PBtoCNF::getNewObjRootLitsAndBlock(uint64_t * objv, uint64_t * objix, int nObj){
  Lit l;
...
void PBtoCNF::evalModel(vec<lbool> &currentModel, uint64_t * objv, uint64_t *ap_objv){
  Lit l;
  for(int di = 0; di < getFormula()->nObjFunctions(); di++){
...
void openwbo::evalToIndex(const YPoint& yp, uint64_t * objix, std::vector<PBtoCNF::rootLits_t>& objRootLits){
