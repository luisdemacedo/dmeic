...
public:
  SlideDrillMO(int verb = _VERBOSITY_MINIMAL_, int weight = _WEIGHT_NONE_,
...
class SlideDrillServerMO : public virtual PBtoCNFServerMO,
                           public virtual SlideDrillMO {
public:
  SlideDrillServerMO(int verb = _VERBOSITY_MINIMAL_, int weight = _WEIGHT_NONE_,
                     int strategy = _WEIGHT_NONE_, int enc = _CARD_MTOTALIZER_,
                     int pb = _PB_SWC_, int pbobjf = _PB_GTE_,
                     int apmode = encoding::_ap_outvars_, float eps = 1,
      ...
      : PBtoCNF(verb, weight, strategy, enc, pb, pbobjf),
        ...
        SlideDrillMO(verb, weight, strategy, enc, pb, pbobjf, apmode, eps,
                     searchStrat, ascend, lower, wl_type) {};

  void build() override { waiting_list->insert(pareto::max); };
