...
public:
  SlideDrillShuntMO(int verb = _VERBOSITY_MINIMAL_, int weight = _WEIGHT_NONE_,
      ...
      : PBtoCNF(verb, weight, strategy, enc, pb, pbobjf),
        ConflictShuntMO(verb, weight, strategy, enc, pb, pbobjf, apmode, eps,
                        searchStrat, conf_budget) {
    upper = make_shared<SlideDrillServerMO>(verb, weight, strategy, enc, pb,
                                            pbobjf, apmode, eps, searchStrat,
...
public:
  UnsatSatShuntMO(int verb = _VERBOSITY_MINIMAL_, int weight = _WEIGHT_NONE_,
                  ...
                  int apmode = encoding::_ap_outvars_, float eps = 1,
                  int searchStrat = 3, int conf_budget = INT32_MAX,
                  bool ascend = false, bool lower_b = false, int wl_type = 0)
      : PBtoCNF(verb, weight, strategy, enc, pb, pbobjf),
        ConflictShuntMO(verb, weight, strategy, enc, pb, pbobjf, apmode, eps,
                        searchStrat, conf_budget) {
    lower = make_shared<UnsatSatListMO>(verb, weight, strategy, enc, pb, pbobjf,
                                        apmode, eps, searchStrat, ascend,
                                        lower_b, wl_type);
...
public:
  SlideUnsatSatHonerShuntMO(int verb = _VERBOSITY_MINIMAL_,
                            ...
                            int apmode = encoding::_ap_outvars_, float eps = 1,
                            int searchStrat = 3, int conf_budget = INT32_MAX,
                            bool ascend = false, bool lowerb = false,
                            int wl_type = 0, bool core_ascend = false,
                            bool core_optim = true, bool core_block = true)
      : PBtoCNF(verb, weight, strategy, enc, pb, pbobjf),
        ConflictShuntMO(verb, weight, strategy, enc, pb, pbobjf, apmode, eps,
                        searchStrat, conf_budget) {
    upper = make_shared<SlideDrillServerMO>(verb, weight, strategy, enc, pb,
                                            pbobjf, apmode, eps, searchStrat,
                                            ascend, lowerb, wl_type);

    lower = make_shared<UnsatSatHonerServerMO>(
        verb, weight, strategy, enc, pb, pbobjf, apmode, eps, searchStrat,
        core_ascend, core_optim, core_block);
    optim = lower.get();
  }
};

class SlideDrillUnsatSatShuntMO : public ConflictShuntMO,
                                  public virtual Bounded {
public:
  SlideDrillUnsatSatShuntMO(int verb = _VERBOSITY_MINIMAL_,
                            int weight = _WEIGHT_NONE_,
                            int strategy = _WEIGHT_NONE_,
                            int enc = _CARD_MTOTALIZER_, int pb = _PB_SWC_,
                            int pbobjf = _PB_GTE_,
    ...
    lower =
        make_shared<UnsatSatListMO>(verb, weight, strategy, enc, pb, pbobjf,
                                    apmode, eps, searchStrat, true, false, 2);
    optim = lower.get();
  }
...
public:
  UnsatSatListPMinimalMO(int verb = _VERBOSITY_MINIMAL_,
                         int weight = _WEIGHT_NONE_,
                         int strategy = _WEIGHT_NONE_,
                         int enc = _CARD_MTOTALIZER_, int pb = _PB_SWC_,
                         int pbobjf = _PB_GTE_,
    ...
    setConflictLimit(conf_budget);
    upper =
        make_shared<PMinimalServerMO>(verb, weight, strategy, enc, pb, pbobjf);

    lower = make_shared<UnsatSatListMO>(verb, weight, strategy, enc, pb, pbobjf,
                                        apmode, eps, searchStrat, false, false,
                                        wl_type);
    optim = upper.get();
  }
};
