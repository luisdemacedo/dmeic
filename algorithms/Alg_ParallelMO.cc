...
//   for (auto &w : workers)
//     for (int i = w.objRootLits.size(); i < getFormula()->nObjFunctions();
//     i++)
...
//   //             ith_orl.push(
//   //                 std::pair<uint64_t, Lit>(factor * rit->first + 1,
//   //                 rit->second));
...
//   //           for (encoding::wlit_mapt::reverse_iterator rit =
//   //           rootLits.rbegin();
...
//   //             if (ith_orl.size() > j + 1) {
//   //               // printf("Lit %lu %lu\n", rit->first,
//   //               objRootLits[i][j+1].first); p = ith_orl[j + 1].second;
//   //             } else {
//   //               // printf("Lit %lu\n", rit->first);
