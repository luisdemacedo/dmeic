...
public:
  UnsatSatMO(int verb = _VERBOSITY_MINIMAL_, int weight = _WEIGHT_NONE_,
...
class UnsatSatIncMO : public virtual UnsatSatMO,
                      public virtual PBtoCNFServerMO {
public:
  UnsatSatIncMO(int verb = _VERBOSITY_MINIMAL_, int weight = _WEIGHT_NONE_,
                int strategy = _WEIGHT_NONE_, int enc = _CARD_MTOTALIZER_,
...
public:
  UnsatSatIncXsMO(int verb = _VERBOSITY_MINIMAL_, int weight = _WEIGHT_NONE_,
                  int strategy = _WEIGHT_NONE_, int enc = _CARD_MTOTALIZER_,
                  int pb = _PB_SWC_, int pbobjf = _PB_GTE_,
  ...
  bool buildWorkFormula() override {
    ...
    for (int i = 0; i < (int)objRootLits.size(); i++) {
      ...
      objRootLits[i] =
          std::make_shared<rootLits::RootLitsSliced>(rootLits::RootLitsSliced{
              std::move(old), *getFormula()->getObjFunction(i)});
      ...
      getFormula()->replaceObjFunction(
          i, std::make_unique<PBObjFunction>(PBObjFunction{}));
    }
    return result;
  }
...
public:
  UnsatSatIncObjMO(int verb = _VERBOSITY_MINIMAL_, int weight = _WEIGHT_NONE_,
                   int strategy = _WEIGHT_NONE_, int enc = _CARD_MTOTALIZER_,
                   int pb = _PB_SWC_, int pbobjf = _PB_GTE_)
      : PBtoCNF(verb, weight, strategy, enc, pb, pbobjf),
        UnsatSatMO(verb, weight, strategy, enc, pb, pbobjf) {}
