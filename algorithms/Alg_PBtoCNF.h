...
namespace openwbo {
  ...
  public:
    ...
    PBtoCNF(int verb = _VERBOSITY_MINIMAL_, int weight = _WEIGHT_NONE_, int strategy = _WEIGHT_NONE_, 
        //int limit = INT32_MAX,
...
//     encoder.setPBEncoding(pb);
    ...
    void setConflictLimit(int limit) { conflict_limit = limit; nConflicts = conflict_limit; }
...
//     IGTE igtes[MAXDIM]; //max MAXDIM objectives
    ...
    invRootLits_t invObjRootLits = make_shared<std::map<int, int>>(std::map<int, int>{});
...
//     uint64_t reductionFactor[MAXDIM];
  ...
  void evalToIndex(const YPoint& yp, uint64_t * objix, std::vector<PBtoCNF::rootLits_t>& objRootLits);
