#ifndef ALG_UNSATSATEPSILONMO_H
#define ALG_UNSATSATEPSILONMO_H
...
namespace epsilon{
class UnsatSatEpsilonMO: public virtual UnsatSatMO, public virtual PBtoCNF{
protected:
  using conflict_t = vec<Lit>;
  constexpr static bool polarity = false; // because it is an unsat-sat solver
public:
  UnsatSatEpsilonMO(int verb = _VERBOSITY_MINIMAL_, int weight = _WEIGHT_NONE_, int strategy = _WEIGHT_NONE_,
...
protected:
  ...
  pareto::ThinSet<YPoint> fence;
  pareto::ThinSet<YPoint> corner;
  pareto::ThinSet<YPoint> e_sol;
  // lower bound set of original instance
  pareto::ThinGapSet<YPoint> e_gbs;
  map<YPoint, YPoint> e_gbs_t;
